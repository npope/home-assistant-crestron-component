...
from .const import HUB, DOMAIN, CONF_TO_HUB, CONF_FROM_HUB, CONF_JOIN, CONF_SCRIPT
...
class ControlSurfaceSync:
    ...
    async def sync_joins_to_hub(self):
        ...
        for join, template in self.to_hub.items():
            ...
            if join[:1] == "a":
                _LOGGER.debug(
                    f"template_change_callback setting analog join {int(join[1:])} to {int(result)}"
                )
                self.hub.set_analog(int(join[1:]), int(result))
            # Serial Join
            if join[:1] == "s":
                _LOGGER.debug(
                    f"template_change_callback setting serial join {int(join[1:])} to {str(result)}"
                )
